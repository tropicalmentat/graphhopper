...
public class DefaultTagParserFactory implements TagParserFactory {
    @Override
    public TagParser create(EncodedValueLookup lookup, String name, PMap configuration) {
        name = name.trim();
        ...
        if (Roundabout.KEY.equals(name))
            ...
            return new OSMRoadAccessParser(lookup.getEnumEncodedValue(RoadAccess.KEY, RoadAccess.class));
        else if (name.equals("car_access"))
            throw new UnsupportedOperationException("need to pass on lookup");
